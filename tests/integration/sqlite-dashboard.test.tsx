...
const migrations = ['001-initial.sql', '002-runs-and-results.sql', '003-summary-history.sql', '004-result-review.sql', '005-result-comment.sql', '006-broker-contact.sql', '007-result-mismatch-reason.sql'].map((filename, index) => ({ version: index + 1, sql: readFileSync(`migrations/${filename}`, 'utf8') }));
...
describe('SQLite dashboard foundation', () => {
  it('records the first seed once across relaunches and maps the latest completed summary for display', async () => {
    ...
    firstRuns.seed();
    expect(first.db.prepare('PRAGMA user_version').get()).toMatchObject({ user_version: 7 });
